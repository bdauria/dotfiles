...
let getAllExecInfo = (() => {
  var _ref = (0, _asyncToGenerator2.default)(function* (args, root, execInfoContainer, options = {}) {
    ...
    return {
      ...
      options: _extends({}, execInfo.execOptions, options),
...
var _nice = require('nuclide-commons/nice');
...
var _FlowIDEConnection = require('./FlowIDEConnection');

var _FlowIDEConnectionWatcher = require('./FlowIDEConnectionWatcher');
...
class FlowProcess {
  ...
  constructor(root, execInfoContainer) {
    this._subscriptions = new _UniversalDisposable2.default();
    ...
    this._root = root;
    this._isDisposed = new _rxjs.BehaviorSubject(false);

    this._optionalIDEConnections = new _rxjs.BehaviorSubject(null);
    ...
    }, { shouldStartPinging: false, previousState: _FlowConstants.ServerStatus.UNKNOWN }).filter(({ shouldStartPinging }) => shouldStartPinging).subscribe(() => {
      this._pingServer();
    });
  ...
  _createIDEConnectionStream() {
    ...
    const isFailed = this._serverStatus.map(x => x === _FlowConstants.ServerStatus.FAILED).distinctUntilChanged();
    ...
    return shouldStart.switchMap(() => this._createSingleIDEConnectionStream()).takeUntil(this._isDisposed.filter(x => x)).concat(_rxjs.Observable.of(null))
  ...
  _createSingleIDEConnectionStream() {
    ...
    return _rxjs.Observable.fromEventPattern(
    // Called when the observable is subscribed to
    handler => {
      logger.info('Got a subscriber for the Flow IDE connection stream');
      (0, _assert2.default)(connectionWatcher == null);
      connectionWatcher = new _FlowIDEConnectionWatcher.FlowIDEConnectionWatcher(this._tryCreateIDEProcess(), handler);
  ...
  _tryCreateIDEProcess() {
    return _rxjs.Observable.defer(() => this._serverIsReady()).switchMap(serverIsReady => {
      if (!serverIsReady) {
  ...
  execFlow(args, options, waitForServer = false, suppressErrors = false) {
    ...
    return (0, _asyncToGenerator2.default)(function* () {
      ...
      if (_this._serverStatus.getValue() === _FlowConstants.ServerStatus.FAILED) {
        return null;
      }
      for (let i = 0;; i++) {
        try {
          ...
          const couldRetry = [_FlowConstants.ServerStatus.NOT_RUNNING, _FlowConstants.ServerStatus.INIT, _FlowConstants.ServerStatus.BUSY].indexOf(_this._serverStatus.getValue()) !== -1;
  ...
  _startFlowServer() {
    ...
    return (0, _asyncToGenerator2.default)(function* () {
      ...
      if (flowExecInfo == null) {
        ...
        _this2._setServerStatus(_FlowConstants.ServerStatus.NOT_INSTALLED);
        return;
      }
      const lazy = [];
      if ((0, _config.getConfig)('lazyServer')) {
        lazy.push('--lazy');
      }
      ...
      // ChildProcess object.
      // eslint-disable-next-line no-await-in-loop
      const serverProcess = yield (0, _nice.niceSafeSpawn)(flowExecInfo.pathToFlow, ['server', ...lazy, '--from', 'nuclide', '--max-workers', _this2._getMaxWorkers().toString(), _this2._root], flowExecInfo.execOptions);
      const logIt = function logIt(data) {
  ...
  _serverIsReady() {
    ...
    return this._serverStatus.filter(x => x === _FlowConstants.ServerStatus.READY).map(() => true).race(_rxjs.Observable.of(false).delay(SERVER_READY_TIMEOUT_MS))
