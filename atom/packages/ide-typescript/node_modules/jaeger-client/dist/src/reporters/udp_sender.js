...
var UDPSender = function () {
  ...
  _createClass(UDPSender, [{
    ...
    key: '_invokeCallback',
    value: function _invokeCallback(callback, numSpans, error) {
      if (callback) {
    ...
    value: function append(span, callback) {
      ...
      if (spanSize > this._maxSpanBytes) {
        this._invokeCallback(callback, 1, 'span size ' + spanSize + ' is larger than maxSpanSize ' + this._maxSpanBytes);
        return;
      }
      ...
      this.flush(function (numSpans, err) {
        ...
        _this2._invokeCallback(callback, numSpans, err);
      });
    }
  }, {
    key: 'flush',
    value: function flush(callback) {
      ...
      this._client.send(thriftBuffer, 0, thriftBuffer.length, this._port, this._host, function (err, sent) {
        if (err) {
          ...
          _this3._invokeCallback(callback, numSpans, error);
        } else {
          _this3._invokeCallback(callback, numSpans);
        }
      });
    }
  }, {
