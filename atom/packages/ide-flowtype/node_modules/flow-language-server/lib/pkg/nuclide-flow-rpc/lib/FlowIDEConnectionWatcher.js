...
const defaultIDEConnectionFactory = (proc, fileCache) => new _FlowIDEConnection.FlowIDEConnection(proc, fileCache);
...
class FlowIDEConnectionWatcher {
  ...
  _makeIDEConnection() {
    ...
    return (0, _asyncToGenerator2.default)(function* () {
      (0, _log4js.getLogger)('nuclide-flow-rpc').info('Attempting to start IDE connection...');
      let proc = null;
      ...
      while (true) {
        ...
        if (proc != null || attemptEndTime > endTimeMS) {
          ...
          (0, _log4js.getLogger)('nuclide-flow-rpc').info('Failed to start Flow IDE connection... retrying');
          ...
          if (additionalWaitTime > 0) {
            (0, _log4js.getLogger)('nuclide-flow-rpc').info(`Waiting an additional ${additionalWaitTime} ms before retrying`);
            // eslint-disable-next-line no-await-in-loop
      ...
      if (proc == null) {
        (0, _log4js.getLogger)('nuclide-flow-rpc').error('Failed to start Flow IDE connection too many times... giving up');
        return;
      }
      ...
      _this._currentIDEConnectionSubscription = ideConnection.onWillDispose(function () {
        ...
        if (connectionAliveTime < IDE_CONNECTION_HEALTHY_THRESHOLD_MS) {
          ...
          if (_this._consecutiveUnhealthyConnections >= MAX_UNHEALTHY_CONNECTIONS) {
            (0, _log4js.getLogger)('nuclide-flow-rpc').error('Too many consecutive unhealthy Flow IDE connections... giving up');
            return;
          }
        } else {
...
//# sourceMappingURL=FlowIDEConnectionWatcher.js.map