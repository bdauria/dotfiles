...
var _string = require('nuclide-commons/string');
...
class MultiProjectLanguageService {
  ...
  findReferences(fileVersion, position) {
    return _rxjs.Observable.fromPromise(this._getLanguageServiceForFile(fileVersion.filePath)).concatMap(ls => ls.findReferences(fileVersion, position).refCount()).publish();
  }
  ...
  getAdditionalLogFiles(deadline) {
    ...
    return (0, _asyncToGenerator2.default)(function* () {
      ...
      const results = yield Promise.all(roots.map((() => {
        var _ref2 = (0, _asyncToGenerator2.default)(function* (root) {
          try {
            const service = yield (0, _promise.timeoutAfterDeadline)(deadline, _this10._processes.get(root));
            if (service == null) {
            ...
            return [{ title: root, data: (0, _string.stringifyError)(e) }];
          }
        });
